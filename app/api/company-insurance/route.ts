...
export async function POST(req: Request) {
  ...
  try {
    const body = await req.json().catch(() => ({})) as { companyId?: string; policy?: any }
    const { companyId, policy } = body
    ...
    const resp = await fetch(`${apiBase}/companies/${companyId}/insurance`, {
      method: 'POST',
    ...
    return Response.json({ message: e?.message || 'company insurance proxy failed' }, { status: 500 })
  }
}
