...
export async function POST(req: Request) {
  ...
  try {
    const body = await req.json().catch(() => ({})) as { companyId?: string; document?: any }
    const { companyId, document } = body
    ...
    const resp = await fetch(`${apiBase}/companies/${companyId}/documents`, {
      method: 'POST',
    ...
    return Response.json({ message: e?.message || 'company documents proxy failed' }, { status: 500 })
  }
}
